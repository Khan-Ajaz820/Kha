// wasm_glue.js (compatible with both main thread and Web Worker)
(function () {
  // Use 'self' in workers, 'window' in main thread
  const globalScope = typeof window !== 'undefined' ? window : self;
  
  let wasmInstance = null;
  let wasmExports = null;
  globalScope.emojiSrcs = []; // global list of sprite filenames used by wasm
  globalScope.wasmReady = false;
  ...
  async function loadWasmModule(wasmUrl = "build/full_mosaic.wasm") {
    ...
    wasmInstance = instance; // Fixed: was 'wasm'
    wasmExports = instance.exports;
    globalScope.wasmReady = true;
  ...
  async function loadKDTreeIntoWasm(kdJsonUrl = "kd_tree.json") {
    ...
    function flatten(node) {
      ...
      if (node.point && node.point.src) {
        ...
        if (srcMap.has(s)) srcIndex = srcMap.get(s);
          ...
          srcIndex = globalScope.emojiSrcs.length;
          globalScope.emojiSrcs.push(s);
    ...
    return { nodesCount: nodes.length, rootIndex, emojiSrcs: globalScope.emojiSrcs };
  }
  ...
  function runMatchingAndGetResults(colors) {
    ...
    for (let i = 0; i < tileCount; i++) {
      ...
      out[i] = (srcIdx >= 0 && srcIdx < globalScope.emojiSrcs.length) ? globalScope.emojiSrcs[srcIdx] : null;
    }
    return out;
  }

  // expose as globals (works in both window and worker)
  globalScope.loadWasmModule = loadWasmModule;
  globalScope.loadKDTreeIntoWasm = loadKDTreeIntoWasm;
  globalScope.calculateTileColorsFromCanvas = calculateTileColorsFromCanvas;
  globalScope.runMatchingAndGetResults = runMatchingAndGetResults;
})();